...
export default class extends React.Component {
  ...
  static async getInitialProps({ ctx, req, res, query, asPath }) {
    try {
      let [reqDestination, reqExtras] = await Promise.all([
        request(endPoints1.get_destination, null, null, "es"),
        request(endPoints2.get_extras, null, null, "es"),
      ]);
      if (reqDestination.status >= 400 || reqExtras.status >= 400) {
        var status = null;
        if (reqDestination.status != 400) status = reqDestination.status;
        if (reqExtras.status != 400) status = reqExtras.status;
      ...
      console.log("dataDestination index page >>>. ", dataDestination);
  ...
  render() {
    ...
    return (
      ...
      >
        <Section1 dataDestination={dataDestination} />
        <Section2 />
        <Section4 dataDestination={dataDestination} />
        <Section3 />
      </Layout>
    );
  }
