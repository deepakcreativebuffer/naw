...
const link1 =
  "https://www.tripadvisor.com.mx/Attraction_Review-g150812-d13354576-Reviews-Naw-Playa_del_Carmen_Yucatan_Peninsula.html";
...
class Layout extends Component {
  ...
  componentDidMount = async () => {
    ...
    document
      .getElementById("id-cont-gallery")
  ...
  componentDidUpdate = async (prevProps, prevState) => {
    if (prevProps.valueLanguage != this.props.valueLanguage) {
      ...
      let dataDetail = await request(
        endPoints.get_adventure_detail,
        null,
        uuid,
        valueLanguage
      );
      ...
      Router.pushRoute("detail", {
        destination: slug(destination.name).toLowerCase(),
        adventure: slug(adventureType.name).toLowerCase(),
        tour: slug(title).toLowerCase(),
        uuid: uuid,
      });
    ...
    if (
      ...
      prevState.sliders != this.state.sliders
    ) {
      autoHeightMinMiddle("class-autoheight");
    }
  ...
  handleInformation = async (first = true) => {
    ...
    if (typeof timetable != "undefined") {
      if (timetable.length > 0) {
        ...
        dataTour.time = array;
      } else dataTour.time = ["10:00:00"];
    }

    dataTour.title = title;
    dataTour.duration = `${i18n.t("detail:tag_1")}: ${duration} ${i18n.t(
      "reserve:hours"
    )}`;
    ...
    if (typeof extras.destacado != "undefined")
      if (extras.destacado.length > 0)
        dataTour.great = extras.destacado.map((element, i) => {
          let { content } = element;
          return <li key={`li_${i}`}>{content}</li>;
        });
    ...
    if (reviews.length > 0)
      dataTour.reviews = reviews.map((element, i) => {
        ...
        return (
          <div className="sty-content-review" key={`review_${i}`}>
            <div className="sty-uty-font-1-18-2">{tempName}</div>
            ...
            <div className="sty-uty-font-1-14-1">{reviewContent}</div>
          </div>
        );
      });
  ...
  handleSliders(images) {
    ...
    sliders = images.map((element, i) => {
      ...
      return (
        <div className="sty-uti-background sty-detail-img" key={`slider_${i}`}>
  ...
  slideRenderer = (params) => {
    ...
    return (
      <div className={"sty-silders-content"} key={key}>
  ...
  slideRendererResp = (params) => {
    ...
    return (
      <div className={"sty-silders-content class-autoheight"} key={key}>
        {sliders[mod(index, sliders.length)]}
      </div>
    );
  };
  ...
  handleClickSelect = (e) => {
    ...
    let tempindex = index;
    if (direction == "add") tempindex = tempindex + 1;
    if (direction == "remove") tempindex = tempindex - 1;
  ...
  handleForm = async (form) => {
    ...
    if (data != null) {
      ...
      Router.pushRoute(
        `/reservations/${data.object.id}?tour=${btoa(uuid)}&reserveD=${btoa(
          form.date
        )}&reserveH=${btoa(hourDate)}&reserveN=${btoa(form.number)}`
      );
    }
  };
  ...
  render() {
    ...
    if (
      mod(index, sliders.length) > 2 &&
      mod(index, sliders.length) < sliders.length - 3
    ) {
    ...
    return (
      <>
        <div
          className={`sty-detail sty-cont-gallery sty-justify-content ${class1}`}
          id="id-cont-gallery"
        >
          <div className="sty-content-slider fix-1" id="id-gallery-image">
            ...
            />
            <div
              className="sty-arrow-left-slider sty-cursor"
              id="remove"
            ...
            <div
              className="sty-arrow-right-slider sty-cursor"
              id="add"
              onClick={this.handleClickSelect}
            >
                ...
                <>
                  ...
                  <div className="sty-indicators">
                    <div className="sty-cont-indicators">
                      <div
                        className="sty-cont-index"
                        style={{ transform: tranform, width: tranformW }}
                      >
                        {sliders.map((element, i) => {
                          ...
                          return (
                            <div
                              className={`sty-circle ${className}`}
                              key={`indiecator_${i}`}
                            ></div>
                          );
                        })}
        ...
        <section className="sty-content-show-resp">
          <button
            className="sty-button-4 sty-button-w100 sty-uty-font-16-1"
          ...
          <div className="row justify-content-center">
            <div className="col-11 sty-content-detail">
              <div className="row">
                ...
                >
                  <div className="sty-content-section">
                    ...
                    <div className="col-12 sty-padding-sides sty-request-button-1 sty-content-show-resp">
                      <button
                        className="sty-button-1-1 sty-button-w100 sty-uty-font-1-20-1"
                        onClick={this.handleScroll}
                      >
                        {i18n.t("button:button3")}{" "}
                        <MoneyValue price={total}></MoneyValue>
                      </button>
                    </div>
                    <div className="sty-content sty-uty-font-1-15-1">
                      <div>{dataTour.duration}</div>
                      <div>{dataTour.destination}</div>
                      <div>
                        {i18n.t("reserve:bookOnly")} {this.state.prepayPorcent}%{" "}
                        {i18n.t("reserve:inAdvance")}
                      </div>
                    </div>
                  </div>
                  {/* ////// */}
                  {dataTour.great.length > 0 && (
                    <div className="sty-content-section">
                      <div className="sty-title sty-uty-font-1-20-1">
                        {i18n.t("detail:outstanding")}
                      </div>
                      <div className="sty-content sty-uty-font-1-15-1">
                        <ul>{dataTour.great}</ul>
                      </div>
                    </div>
                  )}
                  {/* ////// */}
                  ...
                  <div className="sty-content-section sty-content-show-resp">
                    ...
                    <div
                      className="sty-content-show-arrow"
                      id="id-arrow-1"
                    ...
                    <div
                      className="sty-content sty-uty-font-1-15-1 sty-content-section-resp-1"
                      id="id-show1"
                    >
                      {dataTour.description}
                    </div>
                  ...
                  {/* ////// */}
                  {dataTour.includes.length > 0 && (
                    <div className="sty-content-section">
                      <div className="sty-title sty-uty-font-1-20-1">
                        {i18n.t("detail:includes")}
                      </div>
                      <div className="sty-content sty-uty-font-1-15-1">
                        <ul>{dataTour.includes}</ul>
                      </div>
                    </div>
                  )}
                  {/* ////// */}
                  {dataTour.notincludes.length > 0 && (
                    <div className="sty-content-section">
                      <div className="sty-title sty-uty-font-1-20-1">
                        {i18n.t("detail:notIncludes")}
                      </div>
                      <div className="sty-content sty-uty-font-1-15-1">
                        <ul>{dataTour.notincludes}</ul>
                      </div>
                    </div>
                  )}
                  {/* ////// */}
                  {dataTour.where.length > 0 && (
                    <div className="sty-content-section">
                      <div className="sty-title sty-uty-font-1-20-1">
                        {i18n.t("detail:where")}
                      </div>
                  ...
                  {/* ////// */}
                  {dataTour.requirements.length > 0 && (
                    <div className="sty-content-section">
                      <div className="sty-title sty-uty-font-1-20-1">
                  ...
                  {/* ////// */}
                  {dataTour.recomendations.length > 0 && (
                    <div className="sty-content-section sty-content-hidden-resp">
                      <div className="sty-title sty-uty-font-1-20-1">
                  ...
                  <div className="sty-content-section sty-content-hidden-resp">
                    ...
                    <div className="sty-content sty-uty-font-1-15-1">
                      <ul>
                        <li>{i18n.t("reserve:policy_1")}</li>
                        <li>{i18n.t("reserve:policy_2")}</li>
                        <li>{i18n.t("reserve:policy_3")}</li>
                        <li>{i18n.t("reserve:policy_4")}</li>
                      </ul>
                    </div>
                  </div>
                  <div className="sty-content-section sty-content-show-resp">
                  ...
                  {/* ////// */}
                  {dataTour.reviews.length > 0 && (
                    <div className="sty-content-section sty-fix-1">
                      <div className="sty-title sty-uty-font-1-20-1">
                    ...
                    {sliders.length > 0 && (
                      <>
                        ...
                        <div className="sty-content-hidden-resp">
                          <button
                            className="sty-button-1-1 sty-button-w100 sty-uty-font-16-1"
                            onClick={this.handleShowGallery}
                          >
                            Ver fotos
                          </button>
                ...
                <div
                  className="col-12 order-3 sty-request-resp sty-content-show-resp"
                  id="id-reserve-resp"
                >
